  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1">
        {label}
        {optional && <span className="normal-case font-normal text-slate-400 text-[11px]">(optional)</span>}
      </label>
      {children}
...
export default function ApplyModal({ job, onClose, onSuccess }) {
  ...
  // Fetch profile on mount to pre-fill
  useEffect(() => {
    (async () => {
      try {
              ...
              <Field label="Resume">
                <div className="flex flex-col gap-3">
                  ...
                  <label className={`flex items-start gap-3 p-4 rounded-2xl border-2 cursor-pointer transition
                    ${form.resumeChoice === "profile" ? "border-blue-400 bg-blue-50" : "border-slate-200 hover:border-slate-300 bg-white"}`}
                  >
                    <input
                  ...
                  {/* Upload new */}
                  <label className={`flex items-start gap-3 p-4 rounded-2xl border-2 cursor-pointer transition
                    ${form.resumeChoice === "upload" ? "border-blue-400 bg-blue-50" : "border-slate-200 hover:border-slate-300 bg-white"}`}
                  >
                    <input
                    ...
                    <div className="flex-1">
                      ...
                      {form.resumeChoice === "upload" && (
                        <div
                          ...
                          onClick={(e) => { e.preventDefault(); document.getElementById("apply-resume-input").click(); }}
                        >
                          <input
                            id="apply-resume-input" type="file" accept=".pdf,.doc,.docx"
                            className="hidden"
                            onChange={(e) => {
                          ...
                          />
                          {form.resumeFile ? (
                            <p className="text-sm font-bold text-green-600">✓ {form.resumeFile.name}</p>
                          ) : (
                            <>
              ...
              {job.contact?.email && (
                <div className="flex items-center gap-2.5 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                  ...
                  <p className="text-xs text-slate-500">
                    Application will be sent to <span className="font-semibold text-blue-600">{job.contact.email}</span>
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                  {[
                    ...
                    ["CGPA",         form.cgpa || "—"],
                    ["Resume",       form.resumeChoice === "profile" ? "Profile resume" : form.resumeFile?.name || "Not selected"],
                  ].map(([l, v]) => (
